...
function showPhotoViewer(docId, imageUrl, currentStatus, note) {
    ...
    const createBtn = (text, color, action) => { const btn = document.createElement('button'); btn.textContent = text; btn.style.cssText = `padding:12px 20px; font-size:1.1rem; border-radius:30px; border:none; cursor:pointer; font-weight:bold; color:white; background-color:${color}; box-shadow:0 4px 10px rgba(0,0,0,0.3);`; btn.onclick = action; return btn; };
    const updateStatus = async (newStatus, requireReason = false) => {
        let noteUpdate = {};
        if(requireReason) { const reason = prompt("보류 사유를 입력하세요"); if(reason === null) return; noteUpdate.note = reason; }
        ...
        try { await updateDoc(doc(db, "photo_requests", docId), updateData); viewer.style.display = 'none'; } catch(e) { alert("처리 실패"); }
    };
...
async function startEditMode(item) {
    ...
    container.innerHTML = ""; 
    if(item.options && item.options.length > 0) item.options.forEach(opt => addOptionRow(opt.name, opt.count, opt.price)); else addOptionRow(); 
...
if(btnRegister) {
    btnRegister.addEventListener('click', async () => {
        ...
        optionRows.forEach(row => {
            ...
            if(optName && optPrice) options.push({ id: Date.now() + Math.random().toString(36).substr(2, 5), name: optName, price: optPrice, count: Number(optCount) || 1 });
        });
...
window.addOptionRow = function(name="", count=1, price="") {
    ...
    div.innerHTML = `<input type="text" class="opt-name" placeholder="옵션명" value="${name}" style="flex:2;"><input type="number" class="opt-count" placeholder="입수" value="${count}" style="flex:1; text-align:center;"><input type="text" class="opt-price" placeholder="단가" value="${displayPrice}" style="flex:1.5;"><button class="btn-remove-row" onclick="this.parentElement.remove()">-</button>`;
